...
class HumanInterface(object):
    ...
    def __init__(self, width, height, controller, scale):
        ...
        self._display = pygame.display.set_mode(
            (self._width * scale, self._height * scale), pygame.HWSURFACE | pygame.DOUBLEBUF
        )
    ...
    def run_interface(self, sensor_data, semantic_data, vehicle_data, imu_data, refresh=True):
        ...
        self._surface = pygame.transform.scale(
            self._surface, (self._width * self._scale, self._height * self._scale)
        )
        ...
        radiator_state = (
            "open"
            if self._controller._radiator_cover_state == carla.RadiatorCoverState.Open
            else "closed"
        )
        info_text = [
            "Mission time:       %11s"
            % (str(datetime.timedelta(seconds=int(self._controller.agent._mission_time)))),
            "",
            "IMU data:",
            ...
            " Front Drum:   %4.2f    %4.2f  rad/s"
            % (vehicle_data.front_drums_speed, self._controller.front_drums_target_speed),
            " Front Arm:    %4.2f    %4.2f    rad"
            % (vehicle_data.front_arm_angle, self._controller.front_arm_target_angle),
            " Back Arm:     %4.2f    %4.2f    rad"
            % (vehicle_data.back_arm_angle, self._controller.back_arm_target_angle),
            " Back Drum:    %4.2f    %4.2f  rad/s"
            % (vehicle_data.back_drums_speed, self._controller.back_drums_target_speed),
            " Radiator:     %4.2f   %6s" % (abs(vehicle_data.radiator_cover_angle), radiator_state),
            "",
            "Sensors:              %1s    %1s" % ("C", "L"),
            "%s(1)Front:           %3s  %3d%%"
            % self._controller.get_sensor_info(carla.SensorPosition.Front),
            "%s(2)FrontLeft:       %3s  %3d%%"
            % self._controller.get_sensor_info(carla.SensorPosition.FrontLeft),
            "%s(3)FrontRight:      %3s  %3d%%"
            % self._controller.get_sensor_info(carla.SensorPosition.FrontRight),
            "%s(4)Left:            %3s  %3d%%"
            % self._controller.get_sensor_info(carla.SensorPosition.Left),
            "%s(5)Right:           %3s  %3d%%"
            % self._controller.get_sensor_info(carla.SensorPosition.Right),
            "%s(6)BackLeft:        %3s  %3d%%"
            % self._controller.get_sensor_info(carla.SensorPosition.BackLeft),
            "%s(7)BackRight:       %3s  %3d%%"
            % self._controller.get_sensor_info(carla.SensorPosition.BackRight),
            "%s(8)Back:            %3s  %3d%%"
            % self._controller.get_sensor_info(carla.SensorPosition.Back),
            "",
            "Status:",
            " Current power:      %7.0f Wh" % (self._controller.agent.get_current_power()),
            " Battery percentage:  %8.1f%%"
            % (self._controller.agent.get_current_power() / self._max_power * 100.0),
            " Location:   % 18s"
            % ("(%5.2f, %5.2f)" % (transform.location.x, transform.location.y)),
...
class KeyboardControl(object):
    ...
    def _parse_vehicle_keys(self, keys, milliseconds):
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if event.key == pykeys.K_TAB:
                    self._sensors_status[self._sensor_active][0] = not self._sensors_status[
                        self._sensor_active
                    ][0]
                    self.agent.set_camera_state(
                        self._sensor_active, self._sensors_status[self._sensor_active][0]
                    )
        ...
        if keys[pykeys.K_UP] or keys[pykeys.K_w]:
            self.linear_target_speed = min(
                self.linear_target_speed + self._linear_speed_increase, self._max_linear_speed
            )
        elif keys[pykeys.K_DOWN] or keys[pykeys.K_s]:
            self.linear_target_speed = max(
                self.linear_target_speed - self._linear_speed_increase, -self._max_linear_speed
            )
        else:
            self.linear_target_speed = max(
                self.linear_target_speed - 2 * self._linear_speed_increase, 0.0
            )

        if keys[pykeys.K_RIGHT] or keys[pykeys.K_d]:
            self.angular_target_speed = max(
                self.angular_target_speed - self._angular_speed_increase, -self._max_angular_speed
            )
        elif keys[pykeys.K_LEFT] or keys[pykeys.K_a]:
            self.angular_target_speed = min(
                self.angular_target_speed + self._angular_speed_increase, +self._max_angular_speed
            )
        else:
            self.angular_target_speed = max(
                self.angular_target_speed - 2 * self._angular_speed_increase, 0.0
            )

        if keys[pykeys.K_f]:
            self.front_drums_target_speed = min(
                self.front_drums_target_speed + self._drum_speed_increase, self._max_drum_speed
            )
            self.agent.set_front_drums_target_speed(self.front_drums_target_speed)
        elif keys[pykeys.K_v]:
            self.front_drums_target_speed = max(
                self.front_drums_target_speed - self._drum_speed_increase, -self._max_drum_speed
            )
            self.agent.set_front_drums_target_speed(self.front_drums_target_speed)

        if keys[pykeys.K_g]:
            self.front_arm_target_angle = min(
                self.front_arm_target_angle + self._arm_angle_increase, self._max_arm_angle
            )
            self.agent.set_front_arm_angle(self.front_arm_target_angle)
        elif keys[pykeys.K_b]:
            self.front_arm_target_angle = max(
                self.front_arm_target_angle - self._arm_angle_increase, -self._max_arm_angle
            )
            self.agent.set_front_arm_angle(self.front_arm_target_angle)
        if keys[pykeys.K_h]:
            self.back_arm_target_angle = min(
                self.back_arm_target_angle + self._arm_angle_increase, self._max_arm_angle
            )
            self.agent.set_back_arm_angle(self.back_arm_target_angle)
        elif keys[pykeys.K_n]:
            self.back_arm_target_angle = max(
                self.back_arm_target_angle - self._arm_angle_increase, -self._max_arm_angle
            )
            self.agent.set_back_arm_angle(self.back_arm_target_angle)
        if keys[pykeys.K_j]:
            self.back_drums_target_speed = min(
                self.back_drums_target_speed + self._drum_speed_increase, self._max_drum_speed
            )
            self.agent.set_back_drums_target_speed(self.back_drums_target_speed)
        elif keys[pykeys.K_m]:
            self.back_drums_target_speed = max(
                self.back_drums_target_speed - self._drum_speed_increase, -self._max_drum_speed
            )
            self.agent.set_back_drums_target_speed(self.back_drums_target_speed)

        return (
            not self._pygame_window_active,
            carla.VehicleVelocityControl(self.linear_target_speed, self.angular_target_speed),
            self._sensor_active,
        )
