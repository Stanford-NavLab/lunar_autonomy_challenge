...
class FineGrid:
    def __init__(self, N=27, cell_size=0.15, mu_elev=0.0, sigma_elev=1.0):
        ...
        self.grid_size = N // cell_size # grid discretization
        # Store two grids: 
    ...
    def create_grid(self, mu_elev, sigma_elev):
        ...
        elevation_noise = np.random.normal(mu_elev, sigma_elev, size=(self.grid_size, self.grid_size))
    ...
    def update_is_mapped(self, rov_x, rov_y, rov_heading, cameras_on, lights_intensity, h_fov, max_range):
        """
        ...
        """
        camera_heading = rov_heading + np.array([0, 0, np.pi, np.pi, np.pi/2, -np.pi/2, 0, np.pi]) # rad
                ...
                for r in np.linspace(0, max_range * intensity, num=20):
                    for theta in np.linspace(-h_fov / 2, h_fov / 2, num=20):
                        ...
                        if 0 <= x_idx < self.grid_size and 0 <= y_idx < self.grid_size:
                            ...
                            mapping_confidence = max(0, 0.5 * intensity * (1 - r/(max_range * intensity)))
...
class RoverEnv:
    ...
    def _get_state(self):
        ...
        return np.array([self.x,
                         ...
                         self.battery,
                         fraction_visited,
                         self.time_remaining], dtype=float)
