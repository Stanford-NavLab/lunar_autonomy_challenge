...
from lac.utils.general import create_gif, convert_gif_to_mp4
...
def save_waypoints_gif(
    ...
    for i in range(len(waypoints)):
        fig = plot_waypoints(waypoints[: i + 1], show_endpoint=True, dark_mode=dark_mode)
        fig.update_layout(
            width=1000,
            height=1000,
        )
    ...
    create_gif(image_folder, output_gif, duration=int(dt * 1000), loop=1)

    # NOTE: doesn't seem to be working
    if mp4:
        convert_gif_to_mp4(output_gif, f"{path}/{name}.mp4")
